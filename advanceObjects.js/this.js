...
// Everything seems to be working fine. What if we wanted to add a new method
// to our goat object called .diet() that prints the goat‘s dietType?
const goat = {
  dietType: 'herbivore',
  makeSound() {
    console.log('baaa');
...
// That’s because inside the scope of the .diet() method, we don’t automatically have
// access to other properties of the goat object.
// Here’s where the this keyword comes to the rescue. If we change the .diet()
// method to use the this, the .diet() works! :
const goat = {
  dietType: 'herbivore',
  makeSound() {
    console.log('baaa');
...
// object is goat and by using this we’re accessing the goat object itself,
...
// calling object. However, it becomes a bit more complicated when we start using arrow functions
// for methods. Take a look at the example below:
const goat = {
  dietType: 'herbivore',
  makeSound() {
    console.log('baaa');
...
// Notice that in the .diet() is defined using an arrow function.
...
// the global object, or an object that exists in the global scope, which doesn’t have a dietType
// property and therefore returns undefined.
// To read more about either arrow functions or the global object check out the MDN documentation
// of the global object and arrow functions.
